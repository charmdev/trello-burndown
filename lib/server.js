...
main.use('/devstats', function(req, res, next) {
	console.log(settings.devstatsTemplatePath);
    if (req.method === "GET" && req.query.sprint) {
    	console.log(req.query.sprint);
        var sprint = req.query.sprint;
        var helper = new Helper();
        helper.requestDevStats(req.query.sprint, function(data, error)
		{
			if (error === null)
			{
                console.log('ok');
				generateDevStats(sprint, data, function(err, data) {
                    if (err) {
                        console.log(err);
                        return;
                    }
        ...
        generateDevStats(sprint, sprintData, sprintDataPath, function(err, data) {
            if (err) {
                console.log(err);
                return;
            }
...
function generateDevStats(sprint, rawData, callback) {
    var dataForTemplate = {
        title: settings.html_title,
        header: settings.html_header,
        sprint: sprint,
		statsData: JSON.stringify(generateDevStatsData(rawData))
    };

    var templatedData = '';
    var stream = mu.compileAndRender(settings.devstatsTemplatePath, dataForTemplate)
        .on('data', function(data) { templatedData += data.toString(); })
        .on('error', function(error) { callback(error); })
        .on('end', function() {
...
function generateDevStatsData(rawData){
	return rawData;
}
...
function generateOutput(sprint, sprintData, sprintDataPath, callback) {
	var dataForTemplate = {
		...
		devStats: formatDevStats(sprintData.extended.statisticsSummary.devStats)
	};

	console.log(dataForTemplate.devStats);
...
function formatDevStats(devStats) {
	var result = [];
    for (var key in devStats)
	{
		result.push({"stat": key + ": " + JSON.stringify(devStats[key])});
	}
